          ...
          if (session == '') {
            api.ritm('*', userId, 'datyp', 'passcode', function(res) {
              ...
              if (passCodeId == 'not found') {
                ...
                pageInput('Hello '+ person+', please confirm your identity: ', 'password', 'Password', 'password');
              };
            });
          }
          else {
      ...
      case 'password':
        ...
        api.ritm('*', 0, 'tmplt', 'prson', function(tmplt) {
          ...
          if(prsonTmpltId == 'not found') {
            ...
            api.ritm('*', userId, 'datyp', 'passcode', function(res) {
              passCodeId = res[0]['_id'];
              if (passCodeId == 'not found') {
                main(); //Not possible in theory
              }
              else {
                api.ritm('*', passCodeId, 'value', '*', function(res) {
                  if(res[0]['_id'] == 'not found') {
                    main(); //Not possible in theory
                  }
                  else {
                    var passcode = res[0]['val'];
